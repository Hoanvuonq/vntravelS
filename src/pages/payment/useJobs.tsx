...
const useJobs = () => {
    ...
    const renderTabItems = useMemo(() => {
        return tabItems.map(({ title, classActive, classUnactive, key }: ITabs, index: number) => (
            <div className={`border-b-2 ${isEqual(key, selectedTab) ? classActive : classUnactive} text-content cursor-pointer hover-items`} onClick={(e) => handleTabNavigate(e, key)} key={index}>
                {title}
            </div>
        ));
