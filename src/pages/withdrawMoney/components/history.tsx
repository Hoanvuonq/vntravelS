...
const HistoryWithdraw = () => {
    ...
    const HistoryPaymentMemo = useMemo(() => {
        return historyPayment.map(({ amount, status, createdAt, _id }, index) => (
            <div key={index} className={`${getStatusBgColor(status)} rounded-lg p-4 mb-4 flex items-center justify-between history-transaction`}>
                <div className="flex items-center xl:gap-[2vw] gap-[6vw]">
                    ...
                    <div>
                        ...
                        <p className="text-infoBill text-gray-700">Số Diểm : {amount}</p>
